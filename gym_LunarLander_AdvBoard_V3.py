...
# TESTING
    if episode % test_period == 0:
        ...
        test_text_AdvB_Coop = 'not solved'
        for t in range(max_num_timesteps):
            prob_act_list = np.zeros((n_models, num_actions))
            prob_max = np.zeros((1, n_models))
            action_val_selected = np.zeros((1, num_actions))
            ...
            if done:
                if reward == 100:
                    test_text_AdvB_Coop = 'SOLVED'
                else:
                    test_text_AdvB_Coop = 'not solved'
                break
        solved_Coop_list.append(test_text_AdvB_Coop)
        ...
        print('Training Network Reward: {:.2f}, Advisory Board Reward: {:.2f}, Adv Cooperative Decision: {:.2f}, test_text_AdvB_Coop {}'
              .format(total_points_TrainedNetwork, DQN_models[best_model_index]['last_total_reward'], total_points_dqns_coop, test_text_AdvB_Coop))
        #print('***********************************************************')
        ...
        try:
            with open(r'gym_LunarLander_AdvBTraining_AdvBoard_0995_10Adv_V1.txt', 'w') as fp:
                fp.write('\n'.join(str(item) for item in total_reward_list))
            with open(r'gym_LunarLander_AdvBTraining_TrainingNet_0995_10Adv_V1.txt', 'w') as fp:
                fp.write('\n'.join(str(item) for item in total_reward_test_list))
            with open(r'gym_LunarLander_AdvBTraining_Cooperative_0995_10Adv_V1.txt', 'w') as fp:
               fp.write('\n'.join(str(item) for item in total_reward_adv_coop_list))
            with open(r'gym_LunarLander_AdvBTraining_Cooperative_SOLVED_LIST_0995_10Adv_V1.txt', 'w') as fp:
                fp.write('\n'.join(str(item) for item in solved_Coop_list))
        except:
        ...
        try:
            if episode % 10 == 0:
                with open(r'gym_LunarLander_AdvBoard_wins.txt', 'w') as fp:
                    fp.write('\n'.join(str(item) for item in dqn_performance_wins_list))
                with open(r'gym_LunarLander_AdvBoard_trials.txt', 'w') as fp:
